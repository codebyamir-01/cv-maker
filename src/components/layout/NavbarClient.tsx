...
/**
 * NavbarClient - Client component that shows the correct CTA based on session.
        ...
        <div className="container mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-4">
            {isLoggedIn ? (
              <Link
                href="/dashboard"
                className="inline-flex items-center gap-2 justify-center rounded-full bg-blue-600 hover:bg-blue-700 text-white px-6 h-11 text-[15px] font-semibold shadow-md transition-colors"
              >
              ...
              <Link
                href="/login"
                className="inline-flex items-center justify-center rounded-full bg-blue-600 hover:bg-blue-700 text-white px-8 h-11 text-[15px] font-semibold shadow-md transition-colors"
              >
                Log In
              </Link>
            )}
            ...
            <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-slate-100 overflow-hidden">
              <nav className="flex flex-col p-3 space-y-1">
                ...
                <div className="pt-3 mt-2 border-t border-slate-100 flex flex-col gap-2 px-1 pb-1">
                  {isLoggedIn ? (
                    <Link href="/dashboard" className="inline-flex items-center justify-center gap-2 w-full rounded-xl bg-blue-600 text-white h-11 text-sm font-bold hover:bg-blue-700 transition-colors">
                      <LayoutDashboard className="w-4 h-4" />
                      Go to Dashboard
                    </Link>
                  ) : (
