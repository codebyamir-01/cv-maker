...
const MAX_REQUESTS = 3; // 3 per minute for this heavier endpoint
...
export async function POST(req: Request) {
  try {
    ...
    const resumeString = JSON.stringify(resumeData).substring(0, 10000);

    const systemPrompt = `You are an expert ATS (Applicant Tracking System) algorithm and senior technical recruiter. 
Your task is to analyze the provided candidate resume JSON against the provided Job Description.
